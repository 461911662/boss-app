/*
 * vendor/boss/app/boss1-app/watch-ui/net/pipeline/pipeline.c
 *
...
pipeline_manager_t g_pipeline_manager = {0};
...
int pipeline_start(pipeline_t *p)
{
    ...
    pipeline_process_current(p, -1, NULL);
...
static void pipeline_destroy(pipeline_t *p)
{
    ...
    /* Close timer handle - don't free memory in callback
     * to prevent loop from exiting prematurely */
    if (!uv_is_closing((uv_handle_t *)&p->timeout_timer)) {
...
/* Process events for all active pipelines - called from libuv context */
void pipeline_process_events(int event_id)
{
    uv_mutex_lock(&g_pipeline_manager.lock);
    ...
    while (p) {
        if (p->active && p->current) {
            ...
            if (node_is_waiting_for_event(p->current, event_id)) {
                /* Mark event as received and process the node */
                p->current->triggered_event_id = event_id;
                pipeline_process_current(p, event_id, NULL);
            }
        }
        p = p->next;
    }
...
int pipeline_subscribe_event(pipeline_t *p, int event_id)
{
    /* Simplified version - real implementation would maintain event subscription table */
    (void)p;
    (void)event_id;
    return 0;
}

int pipeline_unsubscribe_event(pipeline_t *p, int event_id)
{
    /* Simplified version */
    (void)p;
    (void)event_id;
...
void pipeline_destroy_async(pipeline_t *p)
{
    ...
    async_handle->data = p;
    uv_async_init(p->loop, async_handle, pipeline_async_destroy_cb);
