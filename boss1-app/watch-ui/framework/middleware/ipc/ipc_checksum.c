/****************************************************************************
 * apps/watch-ui/ipc/ipc_checksum.c
 *
...
static const uint32_t crc32_table[256] =
{
  ...
  0xd80d2bda, 0xaf0a1bbc, 0x36034af6, 0x41047a60,
...
bool ipc_verify_request_checksum(const ipc_request_t *req)
{
  ...
  uint32_t calculated = ipc_calculate_checksum(req->data, req->data_len);

  return req->checksum == calculated;
}

bool ipc_verify_response_checksum(const ipc_response_t *resp)
{
  if (!resp || resp->magic != IPC_MAGIC)
    {
      return false;
    }

  uint32_t calculated = ipc_calculate_checksum(resp->data, resp->data_len);

  return resp->checksum == calculated;
}
