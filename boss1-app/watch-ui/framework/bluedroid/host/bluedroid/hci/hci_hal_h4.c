...
/** HCI硬件抽象层打开函数
 * @details 主要包含以下模块：
 * 1. 创建HAL到HCI的上行环境，下行环境已经在hci_layer_init_env函数中初始化了。
 * 2. 注册HCI层的上下行回调函数到HAL层。
 */
static bool hal_open(const hci_hal_callbacks_t *upper_callbacks, void *task_thread)
{
    ...
    // 注册HCI侧的回调函数，便于HAL的数据包到HCI和发送HCI层的数据包。
...
/** HCI硬件抽象层关闭函数
 * @details 主要包含以下模块：
 * 1. 注销HAL到HCI的上行环境，下行环境已经在hci_layer_init_env函数中初始化了。
 * 2. 注销HCI层的上下行回调函数到HAL层。
 */
static void hal_close(void)
{
    hci_hal_env_deinit();
    hci_host_unregister_callback();
}
