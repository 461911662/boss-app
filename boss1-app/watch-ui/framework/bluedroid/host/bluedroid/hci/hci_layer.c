...
/** @details hci_start_up为HCI逻辑的入口函数，它的主要作用如下：
 * 1、初始化HCI框架中的环境，比如创建命令发送队列等。
 * 2、
 */
...
/**
 * @details 主要封装如下功能：
 *   1、初始化当前host侧的信用为1，当host命令发送时信用将为0，等待controller回复command complete
 *      或者command status时，该信用值才会加1。这个是蓝牙part E，4.4流控规定的，目的是控制主机发
 *      送命令的速率，避免控制器过载。
 *   2、注册命令发送队列，其中有些命令需要ACK，所以要使用蓝牙流控。
 *      当使用蓝牙流控时，就会用到cmd_waiting_q来创建超时定时器，用来跟踪流控的命令是否ACK超时。
 *   3、注册数据包发送队列，由于是数据包或者不需要ACK的命令，所以可以不使用cmd_waiting_q。
 *   4、注册HCI下行数据处理事件downstream_data_ready，作用如下：
 *      如果host信用不为0，直接发送命令
 *      如果命令发送队列条件满足，则分发给命令发送队列处理
 *      如果数据包发送队列条件满足，则分发给数据包发送队列处理
 */
static int hci_layer_init_env(void)
{
...
static pkt_linked_item_t *get_waiting_command(command_opcode_t opcode)
{
    ...
    for (const list_node_t *node = osi_list_begin(cmd_wait_q->commands_pending_response);
        ...
        pkt_linked_item_t *wait_entry = osi_list_node(node);
        if (wait_entry) {
            hci_cmd_metadata_t *metadata = (hci_cmd_metadata_t *)(wait_entry->data);
