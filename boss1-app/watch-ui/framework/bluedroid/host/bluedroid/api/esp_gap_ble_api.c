...
esp_err_t esp_ble_gap_update_conn_params(esp_ble_conn_update_params_t *params)
{
    ...
    ESP_BLUEDROID_STATUS_CHECK(ESP_BLUEDROID_STATUS_ENABLED);
    if(!params) {
        BT_LOG_E("%s,params is NULL", __func__);
        return ESP_FAIL;
    }
    ...
    if (ESP_BLE_IS_VALID_PARAM(params->min_int, ESP_BLE_CONN_INT_MIN, ESP_BLE_CONN_INT_MAX) &&
        ...
        BT_LOG_E("%s,invalid connection params:min_int = %d, max_int = %d, latency = %d, timeout = %d",\
...
esp_err_t esp_ble_gap_addr_create_static(esp_bd_addr_t rand_addr)
{
    ...
    rand_addr[0] = 0xC0 | (random() & 0x3F);
    for (int i = 1; i < 6; i++) {
        rand_addr[i] = random() & 0xFF; // Randomize remaining bits
    }
    return ESP_OK;
}
...
{
    if(esp_bluedroid_get_status() != (ESP_BLUEDROID_STATUS_ENABLED)) {
        BT_LOG_E("%s, bluedroid status error", __func__);
        return ESP_FAIL;
    }
...
uint8_t *esp_ble_resolve_adv_data( uint8_t *adv_data, uint8_t type, uint8_t *length)
{
    if (((type < ESP_BLE_AD_TYPE_FLAG) || (type > ESP_BLE_AD_TYPE_128SERVICE_DATA)) &&
        ...
        BT_LOG_E("the eir type not define, type = %x\n", type);
        return NULL;
    }

    if (adv_data == NULL) {
        BT_LOG_E("Invalid p_eir data.\n");
        return NULL;
    }
        ...
        void *value, uint8_t len)
{
    ...
    if (param_type == ESP_BLE_APP_ENC_KEY_SIZE) {
        BT_LOG_E("ESP_BLE_APP_ENC_KEY_SIZE is deprecated, use ESP_GATT_PERM_ENCRYPT_KEY_SIZE in characteristic definition");
                                                           ...
                                                           const esp_ble_gap_ext_adv_params_t *params)
{
    ...
    if (ESP_BLE_IS_VALID_PARAM(params->interval_min, ESP_BLE_PRIM_ADV_INT_MIN, ESP_BLE_PRIM_ADV_INT_MAX) &&
        ...
        BT_LOG_E("%s,invalid ext adv params", __func__);
        return ESP_ERR_INVALID_ARG;
    }
                                                    ...
                                                    const esp_ble_gap_conn_params_t *phy_coded_conn_params)
{
    ...
    if (phy_mask & ESP_BLE_GAP_PHY_1M_PREF_MASK) {
        ...
        if (ESP_BLE_IS_VALID_PARAM(phy_1m_conn_params->interval_min, ESP_BLE_CONN_INT_MIN, ESP_BLE_CONN_INT_MAX) &&
            ...
            BT_LOG_E("%s,invalid connection params:min_int = %d, max_int = %d, latency = %d, timeout = %d", __func__,
