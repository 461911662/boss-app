...
esp_err_t esp_bluedroid_enable(void)
{
    ...
    if (!bd_already_init) {
        BT_LOG_E("Bludroid not initialised\n");
        return ESP_ERR_INVALID_STATE;
    }

    if (bd_already_enable) {
        BT_LOG_E("Bluedroid already enabled\n");
        return ESP_ERR_INVALID_STATE;
    }
...
esp_err_t esp_bluedroid_disable(void)
{
    ...
    if (!bd_already_enable) {
        BT_LOG_E("Bluedroid already disabled\n");
        return ESP_ERR_INVALID_STATE;
    }
    ...
    *future_p = future_new();
    if (*future_p == NULL) {
        BT_LOG_E("Bluedroid disable failed\n");
        return ESP_ERR_NO_MEM;
    }
...
esp_err_t esp_bluedroid_init_with_cfg(esp_bluedroid_config_t *cfg)
{
    ...
    if (!cfg) {
        BT_LOG_E("%s cfg is NULL", __func__);
        return ESP_ERR_INVALID_ARG;
    }

    if (bd_already_init) {
        BT_LOG_E("Bluedroid already initialised\n");
        return ESP_ERR_INVALID_STATE;
    }
    ...
    if (ret != BT_STATUS_SUCCESS) {
        BT_LOG_E("Bluedroid stack initialize fail, ret:%d", ret);
        return ESP_FAIL;
    }

    /*
     * BTC Init, 负责API与协议profile交互
     */
    ret = btc_init();
    if (ret != BT_STATUS_SUCCESS) {
        BT_LOG_E("Bluedroid Initialize Fail");
        return ESP_FAIL;
    }
...
esp_err_t esp_bluedroid_deinit(void)
{
    ...
    if (!bd_already_init) {
        BT_LOG_E("Bluedroid already de-initialised\n");
        return ESP_ERR_INVALID_STATE;
    }

    if (bd_already_enable) {
        BT_LOG_E("Bludroid already enabled, do disable first\n");
        return ESP_ERR_INVALID_STATE;
    }
    ...
    *future_p = future_new();
    if (*future_p == NULL) {
        BT_LOG_E("Bluedroid de-initialise failed\n");
        return ESP_ERR_NO_MEM;
    }
