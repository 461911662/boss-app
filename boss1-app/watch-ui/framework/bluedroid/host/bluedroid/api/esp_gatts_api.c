                                        ...
                                        uint8_t srvc_inst_id)
{
    ...
    if (max_nb_attr > ESP_GATT_ATTR_HANDLE_MAX) {
        BT_LOG_E("The number of attribute should not be greater than CONFIG_BT_GATT_MAX_SR_ATTRIBUTES\n");
        return ESP_ERR_INVALID_ARG;
    }
                                      ...
                                      uint16_t value_len, uint8_t *value, bool need_confirm)
{
    ...
    if (!gatt_check_connection_state_by_tcb(p_tcb)) {
        BT_LOG_W("%s, The connection not created.", __func__);
        return ESP_ERR_INVALID_STATE;
    }

    if (L2CA_CheckIsCongest(L2CAP_ATT_CID, p_tcb->peer_bda)) {
        BT_LOG_D("%s, the l2cap chanel is congest.", __func__);
        return ESP_FAIL;
    }
...
static esp_err_t esp_ble_gatts_add_char_desc_param_check(esp_attr_value_t *char_val, esp_attr_control_t *control)
{
    if ((control != NULL) && ((control->auto_rsp != ESP_GATT_AUTO_RSP) && (control->auto_rsp != ESP_GATT_RSP_BY_APP))){
            BT_LOG_E("Error in %s, line=%d, control->auto_rsp should be set to ESP_GATT_AUTO_RSP or ESP_GATT_RSP_BY_APP\n",\
                            __func__, __LINE__);
            return ESP_ERR_INVALID_ARG;
    }
    ...
    if ((control != NULL) && (control->auto_rsp == ESP_GATT_AUTO_RSP)){
        if (char_val == NULL){
            BT_LOG_E("Error in %s, line=%d, for stack respond attribute, char_val should not be NULL here\n",\
                            __func__, __LINE__);
            return ESP_ERR_INVALID_ARG;
        } else if (char_val->attr_max_len == 0){
            BT_LOG_E("Error in %s, line=%d, for stack respond attribute,  attribute max length should not be 0\n",\
                            __func__, __LINE__);
            return ESP_ERR_INVALID_ARG;
        }
    }
