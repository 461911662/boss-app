...
tBTM_PM_MCB *btm_pm_sm_alloc(void)
{
    ...
    if (p_db) {
        ...
        if (osi_list_length(btm_cb.p_pm_mode_db_list) >= MAX_L2CAP_LINKS) {
    	    osi_free(p_db);
            p_db = NULL;
        }
        if (!osi_list_append(btm_cb.p_pm_mode_db_list, p_db)) {
    	    osi_free(p_db);
            p_db = NULL;
        }
    }
...
static void btm_pm_check_stored(void)
{
    ...
    for (p_node = osi_list_begin(btm_cb.p_acl_db_list); p_node; p_node = osi_list_next(p_node)) {
	p_acl_cb = osi_list_node(p_node);
...
void btm_pm_proc_mode_change (UINT8 hci_status, UINT16 hci_handle, UINT8 mode, UINT16 interval)
{
    ...
    if (old_state & BTM_PM_STORED_MASK) {
        ...
        for (p_node =(osi_list_begin(btm_cb.p_pm_mode_db_list)); p_node; p_node = (osi_list_next(p_node))) {
	    p_cb = (tBTM_PM_MCB *)osi_list_node(p_node);
