...
BOOLEAN BTM_SecDeleteDevice (BD_ADDR bd_addr, tBT_TRANSPORT transport)
{
     ...
     */
    if (osi_list_is_empty(btm_cb.p_sec_dev_rec_list)) {
        btm_ble_reset_id();
    }
#endif
...
tBTM_SEC_DEV_REC *btm_sec_alloc_dev (BD_ADDR bd_addr)
{
    ...
    for (p_node = osi_list_begin(btm_cb.p_sec_dev_rec_list); p_node; p_node = osi_list_next(p_node)) {
        p_dev_old_rec = osi_list_node(p_node);
    ...
    for (p_node = osi_list_begin(btm_cb.p_sec_dev_rec_list); p_node; p_node = osi_list_next(p_node)) {
        p_dev_new_rec = osi_list_node(p_node);
    ...
    if (!new_entry_found) {
        ...
        if (old_entry_found) {
            ...
            if (malloc_new_entry) {
                osi_list_append(btm_cb.p_sec_dev_rec_list, p_dev_new_rec);
            }
...
void btm_sec_free_dev (tBTM_SEC_DEV_REC *p_dev_rec, tBT_TRANSPORT transport)
{
    ...
    osi_list_remove(btm_cb.p_sec_dev_rec_list, p_dev_rec);
}
...
tBTM_SEC_DEV_REC *btm_find_dev_by_handle (UINT16 handle)
{
    ...
    p_node = osi_list_foreach(btm_cb.p_sec_dev_rec_list, btm_find_sec_dev_in_list, &context);
    if (p_node) {
        p_dev_rec = osi_list_node(p_node);
    }
    return (p_dev_rec);
}
/*******************************************************************************
...
{
    btm_cb.p_sec_dev_rec_list = osi_list_new(osi_free_func);
}
...
{
    osi_list_free(btm_cb.p_sec_dev_rec_list);
}
