...
#endif
    btm_cb.p_acl_db_list = osi_list_new(osi_free_func);
    btm_cb.p_pm_mode_db_list = osi_list_new(osi_free_func);
...
tACL_CONN *btm_bda_to_acl (BD_ADDR bda, tBT_TRANSPORT transport)
{
    ...
    p_node = osi_list_foreach(btm_cb.p_acl_db_list, btm_get_acl_db, (void *)&acl_params);
    if (p_node) {
       p_acl_db = osi_list_node(p_node);
    }
                      ...
                      UINT16 hci_handle, UINT8 link_role, tBT_TRANSPORT transport)
{
    ...
    if (osi_list_length(btm_cb.p_acl_db_list) >= MAX_L2CAP_LINKS) {
        return;
    }
    else {
        p = (tACL_CONN *)osi_malloc(sizeof(tACL_CONN));
	if (p && osi_list_append(btm_cb.p_acl_db_list, p)) {
...
void btm_acl_device_down (void)
{
    ...
    for (list_node_t *p_node = osi_list_begin(btm_cb.p_acl_db_list); p_node;) {
        list_node_t *p_next = osi_list_next(p_node);
        p = osi_list_node(p_node);
        if (p && p->in_use) {
...
UINT16 BTM_GetNumAclLinks (void)
{
    ...
    num_acl = osi_list_length(btm_cb.p_acl_db_list);
    return num_acl;
}
...
{
    osi_list_free(btm_cb.p_acl_db_list);
    osi_list_free(btm_cb.p_pm_mode_db_list);
}
