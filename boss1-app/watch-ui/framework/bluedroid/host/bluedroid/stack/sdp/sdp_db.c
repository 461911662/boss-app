...
tSDP_RECORD *sdp_db_service_search (tSDP_RECORD *p_rec, tSDP_UUID_SEQ *p_seq)
{
    ...
    if (!p_rec) {
	p_node = osi_list_begin(sdp_cb.server_db.p_record_list);
    } else {
	/* get node in the record list with given p_rec */
        p_node = osi_list_get_node(sdp_cb.server_db.p_record_list, p_rec);
	if (p_node == NULL) {
	    return NULL;
	}
	/* get next node */
	p_node = osi_list_next(p_node);
    }

    /* Look through the records. The spec says that a match occurs if */
    /* the record contains all the passed UUIDs in it.                */
    for( ; p_node; p_node = osi_list_next(p_node)) {
        p_rec = osi_list_node(p_node);
...
tSDP_RECORD *sdp_db_find_record (UINT32 handle)
{
    ...
    for(p_node = osi_list_begin(sdp_cb.server_db.p_record_list); p_node; p_node = osi_list_next(p_node)) {
	p_rec = osi_list_node(p_node);
...
UINT32 SDP_CreateRecord (void)
{
	...
	if (p_rec) {
    	    ...
    	    /* Save previous rec */
    	    if (p_db->num_records) {
    	        p_rec_prev = osi_list_back(p_db->p_record_list);
    	    }
    	    /* Append new record */
    	    osi_list_append(p_db->p_record_list, p_rec);
...
BOOLEAN SDP_DeleteRecord (UINT32 handle)
{
    ...
    if (handle == 0 || sdp_cb.server_db.num_records == 0) {
        ...
        for(p_node = osi_list_begin(sdp_cb.server_db.p_record_list); p_node; p_node = osi_list_next(p_node)) {
	    osi_list_remove(sdp_cb.server_db.p_record_list, p_node);
	}
	    ...
	    p_rec = osi_list_node(p_node);
            if (p_rec->record_handle == handle) {
                ...
                osi_list_remove(sdp_cb.server_db.p_record_list, p_rec);
