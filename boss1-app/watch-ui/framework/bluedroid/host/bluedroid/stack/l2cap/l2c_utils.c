...
tL2C_LCB *l2cu_allocate_lcb (BD_ADDR p_bd_addr, BOOLEAN is_bonding, tBT_TRANSPORT transport)
{
    ...
    if(p_lcb == NULL && osi_list_length(l2cb.p_lcb_pool) < MAX_L2CAP_LINKS) {
        p_lcb = (tL2C_LCB *)osi_malloc(sizeof(tL2C_LCB));
	    if (p_lcb) {
	        memset (p_lcb, 0, sizeof(tL2C_LCB));
            list_ret = osi_list_append(l2cb.p_lcb_pool, p_lcb);
	    }else {
...
tL2C_LCB  *l2cu_find_lcb_by_bd_addr (BD_ADDR p_bd_addr, tBT_TRANSPORT transport)
{
    ...
    tL2C_LCB    *p_lcb  = NULL;
    for (p_node = osi_list_begin(l2cb.p_lcb_pool); p_node; p_node = osi_list_next(p_node)) {
        p_lcb = osi_list_node(p_node);
        if ((p_lcb->in_use) &&
#if BLE_INCLUDED == TRUE
                p_lcb->transport == transport &&
#endif
...
tL2C_CCB *l2cu_allocate_ccb (tL2C_LCB *p_lcb, UINT16 cid)
{
    ...
    if(p_ccb == NULL) {
        if (osi_list_length(l2cb.p_ccb_pool) < MAX_L2CAP_CHANNELS) {
            p_ccb = (tL2C_CCB *)osi_malloc(sizeof(tL2C_CCB));

            if (p_ccb) {
                memset (p_ccb, 0, sizeof(tL2C_CCB));
                osi_list_append(l2cb.p_ccb_pool, p_ccb);
            }
        }
     }
    if (p_ccb == NULL) {
...
BOOLEAN l2cu_create_conn (tL2C_LCB *p_lcb, tBT_TRANSPORT transport)
{
    ...
    /* If there is a connection where we perform as a slave, try to switch roles
       for this connection */
    for (p_node = osi_list_begin(l2cb.p_lcb_pool); p_node; p_node = osi_list_next(p_node)) {
        p_lcb_cur = osi_list_node(p_node);
        if (p_lcb_cur == p_lcb) {
            continue;
        }
...
tL2C_CCB *l2cu_find_free_ccb (void)
{
    ...
    for (p_node = osi_list_begin(l2cb.p_ccb_pool); p_node; p_node = osi_list_next(p_node))
    {
        p_ccb = osi_list_node(p_node);
...
void l2cu_check_channel_congestion (tL2C_CCB *p_ccb)
{
    ...
    if (p_ccb->buff_quota != 0) {
        ...
        if ( p_ccb->cong_sent ) {
            ...
            if (q_count > p_ccb->buff_quota || (p_lcb && (p_lcb->link_xmit_data_q) && (osi_list_length(p_lcb->link_xmit_data_q) + q_count) > p_ccb->buff_quota)
#if (CLASSIC_BT_INCLUDED == TRUE)
                    || (p_ccb->peer_cfg.fcr.mode != L2CAP_FCR_BASIC_MODE && q_waiting_ack_count >= p_ccb->our_cfg.fcr.tx_win_sz)
#endif
                    ) {
