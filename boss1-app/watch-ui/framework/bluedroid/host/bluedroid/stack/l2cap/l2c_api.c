...
{
    static osi_sem_t buff_semaphore = NULL ;
    ...
    case L2CA_ADD_BTC_NUM:{
        osi_sem_take(buff_semaphore, OSI_SEM_MAX_TIMEOUT);
        btc_buf ++;
        osi_sem_give(buff_semaphore);
        break;
    }
    ...
    case L2CA_BUFF_INI:{
        btc_buf = 0;
        btu_buf = 0;
        int ret = osi_sem_new(&buff_semaphore, 0, 0);
        if (ret) {
