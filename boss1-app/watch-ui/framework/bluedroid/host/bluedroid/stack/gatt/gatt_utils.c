                                    ...
                                    tBT_TRANSPORT *p_transport)
{
    ...
    if (p_tcb) {
        for(p_node = osi_list_get_node(gatt_cb.p_tcb_list, p_tcb); p_node; p_node = osi_list_next(p_node)) {
	    p_tcb = osi_list_node(p_node);
...
BOOLEAN gatt_is_bda_connected(BD_ADDR bda)
{
    ...
    tGATT_TCB   *p_tcb  = NULL;
    list_node_t *p_node = NULL;
    for(p_node = osi_list_begin(gatt_cb.p_tcb_list); p_node; p_node = osi_list_next(p_node)) {
	p_tcb = osi_list_node(p_node);
        if (p_tcb->in_use &&
...
tGATT_TCB *gatt_tcb_alloc(UINT8 tcb_idx)
{
    ...
    if (p_tcb && osi_list_length(gatt_cb.p_tcb_list) < GATT_MAX_PHY_CHANNEL) {
        memset(p_tcb, 0, sizeof(tGATT_TCB));
        /* Add tcb  block to list in gatt_cb */
        osi_list_append(gatt_cb.p_tcb_list, p_tcb);
        /*  Update tcb id */
...
tGATT_CLCB *gatt_clcb_find_by_conn_id(UINT16 conn_id)
{
    ...
    for(p_node = osi_list_begin(gatt_cb.p_clcb_list); p_node; p_node = osi_list_next(p_node)) {
        p_clcb = osi_list_node(p_node);
...
tGATT_CLCB *gatt_clcb_alloc (UINT16 conn_id)
{
    ...
    if (osi_list_length(gatt_cb.p_clcb_list) < GATT_CL_MAX_LCB) {
	p_clcb = (tGATT_CLCB *)osi_malloc(sizeof(tGATT_CLCB));
	if (p_clcb) {
	   osi_list_append(gatt_cb.p_clcb_list, p_clcb);
...
void gatt_clcb_dealloc (tGATT_CLCB *p_clcb)
{
    if (p_clcb && p_clcb->in_use) {
        ...
        memset(p_clcb, 0, sizeof(tGATT_CLCB));
        osi_list_remove(gatt_cb.p_clcb_list, p_clcb);
	p_clcb = NULL;
    }
}
...
void gatt_cleanup_upon_disc(BD_ADDR bda, UINT16 reason, tBT_TRANSPORT transport)
{
	...
	for(p_node = osi_list_begin(gatt_cb.p_clcb_list); p_node; p_node = p_node_next) {
            p_clcb = osi_list_node(p_node);
            p_node_next = osi_list_next(p_node);
