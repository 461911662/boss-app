...
void GATT_Deregister (tGATT_IF gatt_if)
{
    ...
    for(p_node = osi_list_begin(gatt_cb.p_tcb_list); p_node; p_node = p_next) {
	p_tcb = osi_list_node(p_node);
	p_next = osi_list_next(p_node);
        if (p_tcb->in_use) {
	    ...
	    for(p_node_clcb = osi_list_begin(gatt_cb.p_clcb_list); p_node_clcb; p_node_clcb = p_node_next) {
                p_clcb = osi_list_node(p_node_clcb);
                p_node_next = osi_list_next(p_node_clcb);
                if (p_clcb->in_use &&
