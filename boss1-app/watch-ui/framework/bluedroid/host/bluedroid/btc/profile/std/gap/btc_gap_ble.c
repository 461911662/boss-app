...
uint32_t btc_get_adv_osi_list_length(void);
...
static void btc_process_adv_rpt_pkt(tBTA_DM_SEARCH_EVT event, tBTA_DM_SEARCH *p_data)
{
    ...
    if(btc_check_queue_is_congest()) {
        ...
        if(btc_get_adv_osi_list_length() > BTC_ADV_LIST_MAX_LENGTH || btc_adv_list_count > BTC_ADV_LIST_MAX_COUNT) {
...
void btc_gap_ble_call_handler(btc_msg_t *msg)
{
    ...
    case BTC_GAP_BLE_PERIODIC_ADV_CREATE_SYNC: {
        ...
        #if defined(CONFIG_BT_BLE_FEAT_CREATE_SYNC_ENH)
...
#if SCAN_QUEUE_CONGEST_CHECK
void btc_adv_osi_list_free(void *data)
{
    osi_free(data);
}
