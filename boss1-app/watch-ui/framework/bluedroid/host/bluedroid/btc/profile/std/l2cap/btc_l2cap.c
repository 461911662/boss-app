...
#define VFS_WRITE_TIMEOUT (40) // ms
...
typedef struct {
    ...
    EventBitsGroupHandle tx_event_group;
} l2cap_local_param_t;
    ...
    for (size_t i = 1; i <= BTA_JV_MAX_L2C_CONN; i++) {
        ...
        if ((*slot) == NULL) {
            ...
            if (l2cap_local_param.tx_event_group) {
                eventBitsGroup_clearBits(l2cap_local_param.tx_event_group, SLOT_WRITE_BIT(i) | SLOT_CLOSE_BIT(i));
            }
...
static void l2cap_free_slot(l2cap_slot_t *slot)
{
    ...
    eventBitsGroup_setBits(l2cap_local_param.tx_event_group, SLOT_CLOSE_BIT(slot->serial));
    free_slot_data(&slot->tx);
    free_slot_data(&slot->rx);
    if (slot->close_alarm) {
...
static void btc_l2cap_init(void)
{
    ...
    do {
        ...
        if ((l2cap_local_param.tx_event_group = eventBitsGroupCreate()) == NULL) {
...
void btc_l2cap_cb_handler(btc_msg_t *msg)
{
    ...
    case BTA_JV_DISABLE_EVT:
        ...
        osi_mutex_free(&l2cap_local_param.l2cap_slot_mutex);
        if (l2cap_local_param.tx_event_group) {
            eventBitsGroup_delete(l2cap_local_param.tx_event_group);
            ...
            if (p_data->l2c_write.status == BTA_JV_SUCCESS) {
                ...
                if (p_buf->len == 0) {
                    ...
                    if (fixed_queue_length(slot->tx.queue) <= SLOT_TX_QUEUE_LOW_WM) {
                        eventBitsGroup_setBits(l2cap_local_param.tx_event_group, SLOT_WRITE_BIT(serial));
                    }
                }
